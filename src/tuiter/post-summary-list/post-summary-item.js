    ...
    <li className="list-group-item">
        <div className="row">
            <div className="col-10 pe-3">
                <div>
                    {post.userName !== '' ? <div className= "text-secondary"> {post.userName} <i className="fa-solid fa-circle-check pt-1 ps-1 pe-1"></i> - {post.time} </div> : ''}
                </div>
                {post.topic !== '' ? <div className="fw-bolder"> {post.topic} </div> : ''}
                {post.title !== '' ? <div> {post.title} </div> : ''}
                {post.tweets !== undefined ? <div className="text-secondary"> {post.tweets} Tweets </div> : ''}
            </div>
            <div className="col-2">
